...
```bash {9}
curl -X POST https://api.pushe.co/v2/messaging/notifications/ -H 'authorization: Token YOUR_TOKEN' -H 'content-type: application/json' -d '{"app_ids": "YOUR_APP_ID", "data" :{"title":"عنوان اعلان", "content":"محتوای اعلان"}}
