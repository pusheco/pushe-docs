...
```bash {9}
curl -X POST https://api.pushe.co/v2/messaging/notifications/ -H 'authorization: Token YOUR_TOKEN' -H 'content-type: application/json' -d '{"app_ids": "YOUR_APP_ID", "filters": {"device_id": ["device_id_1","device_id_3", "device_id_2", "device_id_4"], "brand": ["samsung", "sony"], "app_version": ["1.0.1", "1.0.2"]}, "data": {"title": "عنوان پیام", "content": "محتوای پیام"}}'
```
